...
public class FestaController extends AbstractKrController<Festa, FestaDocument> {
    ...
    public @ResponseBody List<FestaDocument> getAll() {
        return this.responseGet(this.service.getAll());
    }

    @ResponseStatus(HttpStatus.OK)
    @GetMapping("/{id}")
    public @ResponseBody FestaDocument getById(@PathVariable final String id) {
        return this.responseGet(this.service.getById(id));
    }

    @ResponseStatus(HttpStatus.CREATED)
    @PostMapping
    public @ResponseBody FestaDocument post(@RequestBody final Festa festa) {
        return this.responsePost(festa, this.service.post(festa));
    }

    @ResponseStatus(HttpStatus.OK)
    @PutMapping("/{id}")
    public @ResponseBody FestaDocument put(@PathVariable final String id, @RequestBody final Festa festa) {
        return this.responsePut(festa, this.service.put(festa, id));
    }
}
